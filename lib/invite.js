...
function validatePermissions (data) {
  ...
  return new Promise((resolve, reject) => {
    if (!data) {
      return reject(new Error('Error - hubo un problema encontrando tu cuenta'))
    }
    ...
    if (!accountAge.isSameOrBefore(validAge)) {
      ...
      let message = `Debes ser miembro :coldev: por ${days} días más para tener privilegios de invitación.`
  ...
  return new Promise((resolve, reject) => {
    ...
    if (res.body.error) {
      ...
      if (res.body.error === 'already_invited') {
        reply = `No puedes invitar a ${guest} por que ya lo invitaron.`
      }

      if (res.body.error === 'already_in_team') {
        reply = `No puedes invitar a ${guest} por que ya tiene cuenta en este Slack.`
      }
    }
