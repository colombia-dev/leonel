...
function invite (bot, message) {
  ...
  return getData(host)
    ...
    .catch(err => {
      ...
      logError('caught', err)
      return bot.reply(message, '😱 ¡Tu invitación no funcionó! Reporta el error en https://github.com/davidalzate/billgates/issues/new')
    })
}
...
function validatePermissions (data) {
  ...
  if (stagingBot && !isMaintainer) {
    return Promise.reject(new Error('Solo los maintainers pueden enviar invitaciones con @billgates-test'))
  }

  debug('validatePermissions')
  return new Promise((resolve, reject) => {
    if (!data) {
      return reject(new Error('Creo que mi base de datos tiene un error, podés reportar esto en https://github.com/davidalzate/billgates/issues/new ?'))
    }
    ...
    if (!accountAge.isSameOrBefore(validAge)) {
      ...
      let message = `Debes ser miembro por ${days} días más pa poder invitar gente.`
...
function handleResponse (res) {
  ...
  return new Promise((resolve, reject) => {
    let reply = '¡Invitación sin pantalla azul!'
